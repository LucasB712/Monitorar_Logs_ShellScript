...
do
        ...
        sed -i 's/User session initialized with token: .*/User session initialized with token: REDACTED/g' "${arquivo}.filtrado"

        sort "${arquivo}.filtrado" -o "${arquivo}.filtrado"
        uniq "${arquivo}.filtrado" > "${arquivo}.unico"
        num_palavras=$(wc -w < "${arquivo}.filtrado")
        ...
        if [[ "$nome_arquivo" == *frontend* ]]; then
                sed 's/^/[Front-end] /' "${arquivo}.unico" >> "${ARQUIVO_DIR}/log_stats_$(date +%F).txt"
        elif [[ "$nome_arquivo" == *backend* ]]; then
                sed 's/^/[Back-end] /' "${arquivo}.unico" >> "${ARQUIVO_DIR}/log_stats_$(date +%F).txt"
        else
